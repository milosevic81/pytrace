...
def parse_material(material_data: dict) -> Material:
    if material_data["type"] == "solid":
        ...
        return CheckerMaterial(color1=parse_color(material_data["color1"]), color2=parse_color(material_data["color2"]))
    else:
        raise ValueError(f"Unknown material type: {material_data['type']}")


def parse_sphere(sphere_data: dict) -> Sphere:
    return Sphere(parse_point(sphere_data["center"]), sphere_data["radius"], parse_material(sphere_data["material"]))


def parse_light(light_data: dict) -> Light:
    return Light(parse_point(light_data["position"]), parse_color(light_data["color"]))
